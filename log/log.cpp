...
void log::write_log(int level, const char *format, ...) {
    ...
    if(m_today != my_tm.tm_mday || m_count % m_split_lines == 0){
        char new_log[256];//日志名
        fflush(m_fp);
        fclose(m_fp);
        char tail[16];//日期后缀
        //生成日期后缀
        snprintf(tail,16,"%d_%02d_%02d_",1900 + my_tm.tm_year,my_tm.tm_mon,my_tm.tm_mday);
        //不是今天
        ...
        //超出最大行数
            ...
            snprintf(new_log,255,"%s%s%s.%lld",dir_name,log_name,tail,m_count / m_split_lines);
        }
    ...
    int n = snprintf(m_buf,48,"%d-%02d-%02d %02d:%02d:%02d.%06ld %s ",
                     my_tm.tm_year + 1900,my_tm.tm_mon,my_tm.tm_mday,
